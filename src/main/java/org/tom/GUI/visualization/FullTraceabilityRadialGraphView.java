 ...
 */
public class FullTraceabilityRadialGraphView extends Display {
...
//    MainWindow main = null;
    ...
    JCheckBox checkBusinessRule;
    JCheckBox teste = new JCheckBox();
    

    
    public FullTraceabilityRadialGraphView(){//MainWindow main) {
...
//        addControlListener(new HoverActionControl("repaint"));
        ...
        checkClass = new JCheckBox("Codes", false);
        ...
        checkBusinessRule = new JCheckBox("Business Rule", false);
        checkBusinessRule.setBackground(Color.white);
        checkBusinessRule.addItemListener(new ItemListener(){
			public void itemStateChanged(ItemEvent e) {
				checkAll.setSelected(false);
				filter();
			}
        });
        byTypeBox.add(checkBusinessRule);
    ...
    public void filter(){
    	...
    	if(checkAll.isSelected()){
    		while (it.hasNext()) {
    			...
    			checkBusinessRule.setSelected(false);
    			VisualItem ed = (VisualItem) it.next();
    			ed.setVisible(true);
    		}
    		m_vis.repaint();
    		...
    		if(!checkActor.isSelected() && !checkFeature.isSelected() && !checkClass.isSelected() && !checkNeed.isSelected() && !checkTestCase.isSelected() && !checkBusinessRule.isSelected()){
    			checkAll.setSelected(true);
    			filter();
    		}
    		else{
				while (it.hasNext()) {  
					...
					if(ed.isInGroup("tree.nodes")){
						...
						case 5:{
							if(checkBusinessRule.isSelected()) ed.setVisible(true);
							else ed.setVisible(false);
							break;
						}
						case 6:{
